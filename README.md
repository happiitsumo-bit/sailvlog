...
- **部外向け公開ビュー** — セッション所有者・Team adminが「学びの要約」を添えて昇格すると、認証不要で開ける読み取り専用ページ `/p/[slug]` が発行される。非公開の注釈・編集UIは一切露出せず、OGP（`unlisted`は`noindex`）にも対応（本番は稼働中だが、**`/p/[slug]`の公開URLは当面部内限定の運用**としており外部へは配布していない — 詳細は下記「デプロイについて」）
...
**Q. 部外向けの公開URLはありますか**
A. 機能としては実装済みで、本番でも動作します。`/sessions/[id]` の「公開する」ボタンから昇格すると、認証不要で開ける読み取り専用ページ `/p/[slug]` が発行されます（手順は上記「部外向け共有（公開ビュー）の使い方」参照）。ただし**現状は ③Quality Gate 未通過のMajor課題が残っているため、`/p/[slug]` の公開URLは部内利用に留め、外部（他大学・部外者）へは配布しない運用制約が敷かれています**（詳細は `docs/dev-org/GATES.md` ③、および後述「デプロイについて」参照）。
...
`v3/replay-mvp` を `main` へ統合したものが、Neon（DB）/ Render（backend）/ Vercel（frontend）の0円構成で本番として稼働しています（2026-07-30統合。`docs/dev-org/GATES.md` ③参照）。ただし ③Quality Gate は本稿執筆時点でまだ通過しておらず（Major課題が残存）、**`/p/[slug]` の公開URLを外部へ配布しない**という運用制約のもとで公開されています。詳細・解除条件は `docs/dev-org/GATES.md` ③を参照してください。

**cold start に注意**: Render無料プランはスリープするため、しばらく誰もアクセスしていないと初回表示に**約12.5秒**かかります（warm状態なら0.33〜0.59秒。`docs/dev-org/GATES.md` ③実測。オーナーが構造的制約として受容済み）。反省会でURLを開く直前に、誰か一人が先に一度開いて温めておくことを推奨します。

**PWA化（ホーム画面に追加）してもcold startは解決しません**: `/`（公開ホーム）はホーム画面に追加してアプリのように起動できますが、これは見た目とURLを開く手間を省くだけで、実体は上記と同じRenderのbackendにアクセスします。ホーム画面から開いた初回も、スリープ明けなら同じ約12.5秒を待つ必要があります。オフラインキャッシュ（Service Worker）は導入していません。

デプロイ作業で実際に踏んだ罠（`docs/dev-org/TASKS.md` 発見事項より。いずれもエラーメッセージから原因に辿り着けない種類だったため記録）:

1. **Renderのサービス作成時、ブランチが既定で `main` に戻ってしまう** — v3の成果が反映されないままデプロイされる。ビルドログの `Checking out commit ... in branch main` で気づける
2. **Dockerfile Path にディレクトリを指定していた** — モノレポ構成のため、Root DirectoryかDocker Build Context Directoryの**どちらか一方**にのみ `backend` を指定する。両方に指定すると `backend/backend` を探して失敗する
3. **Vercelの Framework Preset が `Other` になっていた** — デプロイ自体は「Ready」になるが全パスがプラットフォーム404（`x-vercel-error: NOT_FOUND`）を返す。ビルドログにエラーが出ないため気づきにくい。404本文がNext.jsの404ページでなくプレーンテキストの `NOT_FOUND` なら、アプリのルーティング以前の設定ミスを疑う
4. **Vercel の Deployment Protection（Standard Protection）が本番にも効いていた** — `/` が `vercel.com/sso-api` へ302し、Vercelにログインしていない相手には中身が返らない。プレビューだけの制約と思い込むと詰まる。Settings → Deployment Protection → `Require Log In` をOFFで解除する（Hobbyプランでも操作可能）
