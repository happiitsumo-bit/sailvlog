...
- **実行コマンド**（2026-07-28 READMEへ反映済み）:
  - backend: `cd backend && npm test`（DB作成・migrate・truncateまで自動）
  - frontend(lib/gpx): `cd frontend && npm test`

### 3.1 テスト運用のルール（2026-07-28 確定・オーナー承認済み）

実際に事故を起こした経験から確定させた運用規則。**破ると壊れます。**

1. **テストは同時に2つ以上走らせない。**
   テストDBを共有し、各テストの前に全テーブルを `TRUNCATE` するため、2つのjestプロセスが互いのフィクスチャを消し合う。症状はFK制約違反（`TeamMember_teamId_fkey`）や「403期待→404」など。
   **実際に3回この罠を踏んだ**（エージェント同士のバックグラウンドループ×Team Leadの計測）。計測値が汚染され、誤った結論（「修正で悪化した」）を一度出している。**検証は必ず単独で行い、開始前に `pgrep -f "node_modules/.bin/jest"` がゼロであることを確認する。**

2. **`docker compose exec backend npm test` は使わない。**
   以前はコンテナの `DATABASE_URL`（＝**開発DB** `sailvlog_db`）が使われ、`.env.test` が一度も効いていなかった。つまり**テストのたびに開発データが消えていた**。現在は `setup/env.ts` が `override: true` で `.env.test` を必ず優先するため、コンテナ内からはテストDB（`localhost:5433`）に到達できず失敗する。**これは安全側に倒した意図的な設計**。

3. **本番DBに対して実行しない（多層ガード済み）。**
   `export DATABASE_URL=<本番>` した状態で `npm test` を叩くと本番が全テーブル消去されうる（`dotenv` はシェルの既存環境変数を上書きしないため）。`scripts/ensure-test-db.js` と `helpers/resetDb.ts` の両方で、**接続先ホストが `localhost`/`127.0.0.1`/`db` 以外、またはDB名が `_test` で終わらない場合は中断**する。マイグレーションを本番へ流すときは `export` せず `DATABASE_URL="..." npx prisma migrate deploy` のようにコマンド単位で渡すこと。

4. **flakyを `.skip`／`todo`／リトライ設定で隠さない。**
   CIが不定期に赤くなると「赤は無視してよい」という習慣が生まれ、テストが回帰を捕まえる装置として機能しなくなる。原因を特定して直す。
   **前例**: フルスイート実行時に約1/2の確率でランダムなテストが落ちていた。真因は `supertest` の `request(app)` が**呼び出しのたびにHTTPサーバを生成・破棄する**実装に起因するレースで、リクエストがルートハンドラに到達しないまま404が返っていた（`t31` にデバッグログを仕込み「61回撃ってハンドラ内のログが60回」を観測して確定）。共通ヘルパー `helpers/testServer.ts` を新設し12ファイル全てを移行して根絶。**Team Lead が単独実行で10回連続 133 passed / 0 failed を実測。**
   → **新規テストは必ず `setupTestServer()` を使い、`request(app)` を直接書かないこと。**

5. **検証の分担**（エージェント運用の教訓）:
   長い検証ループは実装担当を作業不能にする（前任者5名が「ループを回しながら待機」で停止した）。
   - **実装担当**: 原因特定と実装。フルスイート実行は**最大2回**まで（単一ファイル実行は何度でも可）
   - **Team Lead**: 10回連続の安定性検証
   個別ファイルで24回連続PASSでも、フルスイートでは別ファイルが落ちる。**個別最適の確認と全体の確認は別物**。

6. **製品コードを変更した修正コミットには回帰テストを同梱する**（③Quality Gate の通過条件）。
   ラウンド1の修正6コミット中4つが無テストで、そのうち1つが新Blocker（B3-01）を作り込んだため確定させた規則。
