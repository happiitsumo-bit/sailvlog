...
- [ ] **未完了。** 下の記入欄がすべて埋まるまで〔E2〕タスクに着手してはならない（GATES ①のGOは「E2本線での設計と〔共通〕作業の先行」を承認したのみで、主役体験は未確定）
- 記入欄（並行検証の完了時にTeam Leadが記入し、オーナーが確認する）:
  - A検証（Googleフォームでカード運用）の計測結果: ＿＿＿＿
  - E2検証（練習2回のスマホGPS収録＋既製ツールで反省会1回）の計測結果: ＿＿＿＿
  - 判定: E2主役 / A主役 / 両FAIL（いずれかに○）: ＿＿＿＿
  - オーナー確認（氏名/手段）: ＿＿＿＿ ／ 日付: ＿＿＿＿
- 判定後の分岐手順:
