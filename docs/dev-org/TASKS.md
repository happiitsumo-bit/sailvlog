...
- [x] T-28: 部内セッションAPIの応答select化 — `publicViewCount` の全経路除外（2026-07-27 architect監査で起票。REVIEW.md R-02の残り半分）
  ...
  - **検証結果（2026-07-28・routine・実態確認）**: コードを確認したところ、成果物①②とも別セッションの後続修正で既に実施済みだった（本タスク自体の実装作業は発生せず、実態確認とチェックボックス補完のみ）。①`GET /api/sessions`一覧は`fix(M-02)`（コミット`ca915d0`）で明示的`select`化済み（`publicViewCount`はselect対象に含まれない）。`POST /api/sessions`・`PATCH /api/sessions/:id`・`GET /api/sessions/:id`は`fix(M3-05)`（コミット`135a26b`）で共通関数`backend/src/lib/serializeSession.ts`に一本化され、戻り値から`publicViewCount`を除外。`POST /:id/unpublish`は元々`{visibility}`のみを手組みで返しており該当なし。②`GET /api/teams/:slug/articles`・`/:slug/questions`は`fix(B-02,M-01)`（コミット`4d14081`）で410 Goneに凍結済み（`backend/src/routes/teams.ts:90-93`）。**検証（本ルーティンで実施）**: ローカルPostgreSQL(port 5433, sailvlog_test)で`npx jest --runInBand`＝**14 suites / 133 passed / 0 failed**（`t97`・`t01-frozen-routes`を含む全件）。`npx jest t97 t01-frozen-routes --runInBand`単独実行でも**2 suites / 17 passed**。`npx tsc --noEmit`＝エラー0。既存の`auth.ts`型エラー2件（過去の発見事項に記載）も別セッションの修正で解消済みで現状ゼロ。
- [ ] T-29: 公開ビュー系フロントの残Major対応〔Codex担当・③Quality Gate再判定の解除条件〕（2026-07-27 architect監査で起票。GATES ③の未解消3件をタスク台帳へ正式登録）
  ...
  - **【体制変更 2026-07-30】担当を「別ブランチの外部担当Codex」から「Team Leadのdev-orgフロー内サブエージェントとしてのCodex」へ変更。** 理由: オーナー指摘「フロントとバックで分けたけど開発スピードがめっちゃ落ちた」。実測として `codex/quality-gate-fixes` ブランチは4日間で**0コミット**だった。作業ブランチを `v3/replay-mvp` に統合し、`codex/quality-gate-fixes` は廃止する（詳細は本ファイル末尾「体制の記録」）

- [ ] T-30: チーム加入APIの新設（**Blocker B3-02 の解消**・ARCH ADR-013）〔2026-07-30 オーナー裁定「これは超重大だから早急に直して」により起票〕
  - 背景: ADR-012で名簿系の機密境界を「そのチームのメンバーか」に変更した結果、**加入する手段がbackendに存在しない**状態になった（`docker compose exec db psql` の直INSERTのみ）。部員10人規模の運用に耐えず、登録直後のユーザーには「壊れていることすら見えない」（`GET /api/teams` が空配列）
  - 成果物: ADR-013 §APIコントラクトの6本 — `POST /api/teams`（作成者がadmin）／`POST /api/teams/join`（**パスにslugを入れない**＝存在オラクル回避、不正コードは404）／`GET /api/teams/:slug/invite`（admin限定・遅延生成）／`POST /:slug/invite/rotate`／`PATCH /:slug/members/:userId`／`DELETE /:slug/members/:userId`（最後のadminは409・自分自身は脱退）。Prismaに `Team.inviteCode String? @unique` ＋ `inviteCodeUpdatedAt` を追加（バックフィルなし）
  - **最も事故りやすい点**: 現行 `GET /api/teams`（findMany）・`GET /api/teams/:slug`（findUnique）は**select無しで全カラム返却**しており、`inviteCode` を足すと一般メンバーのレスポンスに鍵が載る。R-02（`publicViewCount`）・M-02と**完全に同型**。両経路を明示selectに変える
  - 検証: `npx tsc --noEmit` 0件／`npx jest --runInBand` 全件PASS（ベースライン 14 suites・133 passed から減らさない）／`npm run build` 成功。回帰ガードとして「一覧・詳細レスポンスに `inviteCode` キー非含有」「不正コードが404」「rotate後に旧コードで加入不可」「最後のadmin削除で409」をテストで固定
  - 依存: なし（backend単独）。**③Quality Gate再判定の入力**（B3-02はBlocker欄の未解決項目）
  - 未確定の帰結（ADR-013に明記済み）: 招待コードは**転送可能な鍵**であり、部員がLINEに貼ったコードが外部に流れれば部外者が名簿を読める。承認制への移行は「部員数が増える／他大学に配る段階」で再ADR
...
- [ ] BL-02: ネイティブアプリ化（オーナー裁定 2026-07-24: 「ネイティブアプリとして考えます。ただ今ではないのでステイ」。MVPはWebレスポンシブ+既存GPSロガー委託のまま。中間段階のPWA化も未着手。SNS層本格化・全国展開の段階で再検討）
  - **【2026-08-02 再検討済み・ステイ継続を確定】** オーナーの問い「GPXが他のアプリ頼りだけど、機能として取り込みを追加できない?」を契機に RQ-13 として調査（researcher 2観点＋architect統合。`RESEARCH.md` §3.7）。**判定 = (a) 現行維持（既存GPSロガー委託）＋取り込み摩擦の低減。** オーナー裁定でこれを承認（本人発言「GPXデータは既存のアプリで拾って、sailvlogに取り込む方が良さそうですね」）
  - 不採用の理由（要約）: ネイティブ化での2h収録は**条件付きでしか成立しない**（有償SDK $500＋OEM電力管理対応＋年$99・90日ごと再ビルド・OS更新ごと回帰テストの恒常運用）。加えて**年会費停止で既存インストール分ごとアプリが消える**（Apple公式で一次確認）ため、PRD rev.7 のミッション「知識を埋もれさせない」に対し**知識への到達経路を消す構造リスク**を抱え込む。現行構成は知識の正本がWeb/DB側にあり頑健で、業界標準（収録=既存ロガー/専用機・閲覧=Web）とも同型
  - **再開トリガ（この3つのいずれかが実測で成立したら `RESEARCH.md` §4 経由で再検討する）**:
    1. 全国展開・SNS層本格化が到来し、**「既存ロガーの併用が新規ユーザーの脱落点」であることが計測された**場合
    2. **Apple の Fee Waiver 適用が確認され**、かつ代替わり後の年次更新・再ビルドの担い手が**制度として**確保できた場合
    3. 委託先（Geo Tracker 等の無料ロガー）が**軒並み提供終了・有料化**し、委託の前提が崩れた場合
  - **再開時の第一候補は (b) フルネイティブではなく (c) 収録専用の薄いネイティブ**（閲覧はWeb維持）。知識の正本をWebに残す分離構造がミッション整合的で、業界標準（raceQs型）とも同型のため
  - **摩擦の計測が先**（engineering-mindset: 定義→測定→原因→対策）。「収録したのに sailvlog に上がらなかった率」「取り込みの所要時間・手数」を実運用で測る（T-92 の計測系に追加）。摩擦低減への投資判断は実測後に行う
...
### 2026-07-30: フロント/バックのブランチ分割を廃止し、Codexをdev-orgフロー内に取り込む

**変更前**: フロントエンド＝Codex（`codex/quality-gate-fixes` ブランチ・外部担当）／バックエンド＝Claude（`v3/replay-mvp`）／残タスク・テスト＝クラウドルーチン。**技術レイヤーで担当を固定**していた。

**変更後**: **Team Lead（Claude）が両レイヤーの単一の入口**になり、Codexは `codex:codex-rescue` 経由で**同じ作業ツリー内のサブエージェント**として起動する。ブランチは `v3/replay-mvp` に一本化。`codex/quality-gate-fixes` は廃止。

**理由（オーナー指摘 2026-07-30「フロントとバックで分けたけど開発スピードがめっちゃ落ちたので変えたい」）と、計測できた根拠**:

1. **`codex/quality-gate-fixes` は4日間で0コミット。** T-29（③Gateの唯一の解除条件）が誰にも着手されないまま滞留した。レイヤーで分けると、**そのレイヤーの担当が動かない間、Gate全体が止まる**
2. **技術レイヤー分割は `parallel-dev.md` の基準に反していた。** 同ルールは「並列にしてよいのは触るファイルが重ならない**独立タスク**だけ」と定めており、単位は**タスク**であってレイヤーではない。R-03のようにフロントとバックで**1つの契約を両側から実装する**課題は、レイヤーで割ると必ず片側待ちになる（実際にbackend側だけ完成してGateが開かない状態が続いた）
3. **正本を読む主体が分散した。** ADR/正本の解釈が担当ごとにズレ、艇色（ADR-008）のような食い違いの検出が監査まで遅れた

**新しい分離の基準**: レイヤーではなく**触るファイル集合**。同時に走らせるレーンは、ファイルが重ならないことをTeam Leadが起動前に確認する（例: 2026-07-30は T-30=`backend/` ／ T-29=`frontend/` で重複ゼロを確認して並列起動した）。重なるなら分けずに直列にする。

**Codexの使い方**: 「フロント担当」という固定配役ではなく、①仕様が明確で切り出せるタスクの実装 ②Claudeが詰まった箇所の第2意見・別実装 に限定して起動する。起動時は必ず**担当ディレクトリと触ってはいけないディレクトリを明示**する（Team Lead側の実装と衝突させないため）。
...
- （実装者 2026-07-28・T-90 t31-public-api最後のflaky、真因確定）**429行目のt100の仮説（「supertestの`request(app)`が呼び出しごとに新規ephemeralサーバをlisten(0)する実装に起因するレース」）は、同種の症状がt31-public-api.test.tsの④系（レート制限61回連続リクエスト）でも起きることを実測で確定させた**。Team Lead計測（4回中3回PASS/1回FAIL、落ちたのは④のみ）を受け、`lib/rateLimiter.ts`の`check()`と`routes/public.ts`のルートハンドラ入口、`withFrozenClock()`に一時デバッグログ（`T31_DEBUG`環境変数ゲート、コミットには含まれず削除済み）を仕込み、`npx jest t31-public-api --runInBand -t "④..."`を単体で繰り返して失敗を再現した（15〜23回に1回程度の頻度で再現）。
  - 観測事実: `withFrozenClock`が固定した`frozenNow`は61回のリクエストを通じて完全に同一値だった（前任qa-engineerの「時計を止めれば決定的になる」という対策自体は正しく機能していた＝frozen clockの実装は疑わしくない）。
  - 一方、61回`await request(app).get(...)`したにもかかわらず、`checkRateLimit()`内部の`check()`呼び出しログ（=ルートハンドラに到達した回数）が60回しか記録されない失敗ケースを再現した。つまり61回目のHTTPラウンドトリップは**アプリのルートハンドラを経由せず200を返していた**。テスト側のループ回数(`i=0..60`)自体は正しく61回実行されていることも、ループ内に`i`と`status`を出すログを追加して確認済み。
  - これはt100で立てた仮説と完全に同型（このファイルの④〜④-eだけで61回×5テスト=300回超のephemeralサーバがループの中で短間隔に生成/破棄される）。**今回はt100と異なり実際に再現条件を単体テストで安定的に再現できたため、t100の仮説が「確証のない仮説」から「同型バグの2件目の実測確認」に格上げされたと判断した**（frozen clock・並行実行・sweepExpiredなどTeam Leadが既に潰した仮説はいずれも再現ログと矛盾しないことを個別に確認済み＝これらの排除は正しかった）。
  - **対策**: t100と同じ緩和策をt31-public-api.test.ts全体（`request(app)`を使う全28箇所）に適用。`beforeAll`で`app.listen(0)`した実サーバを1つだけ生成し`afterAll`でcloseする形にし、以降は`request(server)`を使い回す（ephemeralサーバ生成をファイル全体で最大300回超→1回に削減）。本番コード（`lib/rateLimiter.ts`・`routes/public.ts`）は一切変更していない（diffなし、既存の挙動は不変）。
  - **検証**: 修正後、単体（`npx jest t31-public-api --runInBand`、フルファイル・16件）を**24回連続PASS**（従来は15〜23回に1回の頻度でFAILしていたため、明確な改善を確認）。`npm test`（フルスイート）を許可された上限の2回実行し、**いずれも14 suites / 133 passed / 0 failed**。`npx tsc --noEmit`エラー0。
  - **申し送り**: t100と合わせてこれで2ファイルが同じ対症療法を個別に適用した状態。もし今後別のテストファイルで同種のflaky（フルスイートでのみ低頻度・単体では再現しにくい・直前リクエストとの間隔が最短の箇所で発生）が見つかった場合は、まず本パターン（`beforeAll`で`server`を1つ生成し`request(server)`へ統一）を疑うのが妥当。恒久対策として「supertestのバージョン確認」「全テストファイル共通の`server`セットアップヘルパーへの集約」はスコープ外のため実装せず記録のみ（Team Lead判断が必要）。

- （実装者 2026-07-28・supertest一時サーバ生成レースの恒久対策）**予告どおりt12でも同型の症状（`Expected: 400 / Received: 404`）が再発したため、Team Lead指示でファイルごとの個別対処をやめ、全ファイル共通の`server`セットアップヘルパーへ集約した**。`backend/src/__tests__/helpers/testServer.ts`に`setupTestServer()`（`beforeAll`で`app.listen(0)`・`afterAll`で必ず`close`し、生成前アクセス時は例外を投げる遅延評価のgetterを返す）を新設し、`request(app)`を使っていた12ファイル全て（health / t01 / t12 / t15 / t30 / t31 / t95 / t96 / t97 / t98 / t99 / t100）を`const getServer = setupTestServer();`＋`request(getServer())`へ移行（t31・t100の既存の個別`beforeAll`/`afterAll`実装もこの共通ヘルパーへ統合し重複を解消）。本番コード（`src/routes/**`・`src/index.ts`）は無変更。
  - **検証**: `grep -rn "request(app)" backend/src/__tests__` は本文コードでは0件（コメント内の説明文のみ残存）。`npx tsc --noEmit`エラー0。`npm run build`成功。`npm test`（フルスイート・許可上限2回のうち1回使用）で**14 suites / 133 passed / 0 failed**、jestは`--detectOpenHandles`なしでも正常終了（ハング・警告なし＝`afterAll`のサーバcloseがリークしていないことの間接確認）。
  - **残った懸念**: 10回連続の安定性検証（今回のフルスイート系flakyの再発有無）はTeam Lead側の担当のため未実施。もし今後も別ファイルで再発する場合、次に疑うべきは「supertestのバージョンアップ／代替ライブラリ」だが、これは承認制のため実装せず提案に留める。

- （routine 2026-07-29・担当割り当てとファイル禁止リストの矛盾・R-09/R-14は未着手）本ルーティンの担当割り当ては「REVIEW.mdのMinor R-09/R-14」を含んでいたが、両方とも「触ってはいけないファイル」リストに載る箇所だった: **R-09**（`frontend/src/components/PublishDialog.tsx:110-118`）は明示的に「Codex担当」と指定されたファイル、**R-14**（`frontend/src/lib/replay/geo.ts:47`・`frontend/src/app/p/[slug]/PublicReplayView.tsx:63-66`）は「`frontend/src/lib/replay/`の描画ロジック（ADR-007）」および「`frontend/src/app/p/**`」として同じく禁止対象（後者はCodex担当ファイル群と同じディレクトリ）。ファイル禁止リストの方が具体的かつ「絶対に手を出さない」と強く指定されているため、割り当てを優先せず**両タスクとも着手しなかった**。担当のズレ（おそらくREVIEW.mdとREVIEW-backend-3.mdの担当分けを設定した時点でR-09/R-14の場所を精査していなかったことが原因と推測）はTeam Lead/オーナー判断で次回の担当割り当てを修正いただきたい。

- （実装者 2026-07-29・REVIEW-backend-3.md Minor m3-05クローズ確認）**コード変更なし、実態確認のみ**。`lib/rateLimiter.ts`を確認した結果、m3-05（レート制限のMapが無制限に増える）は**2026-07-27のm-02修正で既に解消済み**だった。`createFixedWindowLimiter`内部の`check()`/`peek()`双方に確率的遅延掃除（`SWEEP_PROBABILITY`）が実装されており、これを使う全ての固定ウィンドウ・カウンタ（`publicViewLimiter`／`authEmailLimiter`／`authIpLimiter`／本ラウンドで追加した`registerIpLimiter`）が対象に含まれる。`shouldCountView`の`lastViewedAt` Mapも同じ確率的掃除を個別に実装済み。よって「無制限に増える」Mapはコードベース上に存在しない。REVIEW-backend-3.mdの指摘日（2026-07-28）が対象コミット範囲(`309b035..e227dc7`)の敵対的レビューという性質上、m-02修正（`309b035`より前のラウンド1修正群に含まれる）を「ラウンド1見落とし」として再掲したものと考えられる。追加のコード変更・テストは不要と判断した。
