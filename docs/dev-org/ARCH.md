...
1. **加入経路は `POST /api/teams/join` の1本。URLにslugを含めない。**
   ...
   - **【2026-08-02 訂正】** 打ち消し部分は**事実と食い違っていた**（REVIEW-4 M4-02: `POST /api/teams` が既存slugに409を返すため、存在は観測可能だった）。**さらにオーナー裁定により、チームの存在そのものが公開情報となった**（ADR-014）。したがって**409による存在の観測は欠陥ではなく仕様どおりであり、対応不要**。
   - **決定（slugをパスに置かない）自体は維持する。** ただし根拠は「存在の秘匿」ではなく、**①鍵を招待コード1本に集約して認可経路を単純に保つ ②チームの列挙とjoin試行を同一経路で行わせない**の2点に置き換える。守るべきものは存在ではなく**中身**（招待コード・名簿・非公開コンテンツ）である。
...
### ADR-014: チームは既定で公開。秘匿の保証対象を「存在」から「中身（招待コード・名簿・非公開コンテンツ）」へ再定義する

**決定日**: 2026-08-02 / **状態**: 採用（オーナー裁定）/ **関連**: ADR-012・ADR-013（帰結欄を訂正）・REVIEW-4 M4-02

**背景**: REVIEW-4 の M4-02 で「`POST /api/teams` の409が team slug の存在オラクルになっており、ADR-013 の帰結『コードを知らない者はチームの存在すら観測できない』が成立していない」と指摘された。当初 architect は「オプトイン制の公開ページ＋存在オラクルは受容」を案としたが、**オーナーがより単純な裁定を出した**: 「チームの存在自体は誰からも見れるようにしてほしい。基本的にチームは公開だと思う。ただ名簿とかに関しては所属メンバーしか見れない状態にしてほしい」。

**決定**:

1. **チームの存在・チーム名・大学・公開bio・昇格済みの公開物は「公開情報」と定義する。** 全チームに公開ページが既定で存在する（`isPublicPageEnabled` のような有効化フラグは**導入しない**）。
2. **名簿（メンバー一覧）は所属メンバーのみ。** 公開側に名簿を返すAPIを**作らない**。「作らないので漏れない」を構造で担保する。
3. **公開ページは閲覧者に依存しない。** 同一URLで見る人によって内容を変えない。理由: 「自分に見えている＝公開されている」という誤認を生み、かつ**閲覧者非依存であることの自動テストが書けなくなる**。名簿は要ログインの部内画面に一本化する。
4. **オプトアウト列も置かない。** 「公開したくないチーム」の要望が実在した時点で、純追加migrationで可逆に導入できるため、いまは持たない（YAGNI）。
5. **`TeamMember.showOnPublicPage`（掲載同意フラグ）を廃止する。** 公開ページに名簿が存在しない以上、同意の対象がない。**結果として名簿の秘匿は旧案（同意すれば公開）より強くなる。**
6. **`POST /api/teams` の409（slug重複）は仕様どおり**とし、コード変更しない。M4-01 のレート制限は存在秘匿のためではなく**列挙・スパム対策**として維持する。

**却下した代替案**:
- (a) admin オプトイン制（既定 false）— オーナー裁定「チームの存在は誰からも見れるように」と両立しない
- (b) オプトアウト制（既定 true のフラグを持つ）— 「基本的に公開」の実装として過剰。要望が出てから足せる
- (c) 同一URLで閲覧者により出し分け — 上記決定3の理由により却下
- (d) slugをサーバ生成にして存在オラクルを消す — 公開ページURLの可読性（新歓ビラに書ける）と衝突し、そもそも存在が公開情報になったので目的自体が消えた

**選ばなかった側の最強の擁護論**: 「**既定を公開に倒す失敗は取り返しがつかない**（一度クロールされた情報は消えない）。オプトインなら間違えても被害が出ない」— 正当な指摘。反駁は、**公開されるのはチーム名・大学・公開bioという「新歓ビラに書く程度の看板情報」に限られ、個人情報（名簿）を一切含まない**こと。露出する情報の質が旧案の懸念（名簿の公開）とは異なる。

**結果として受け入れるデメリット**: チーム名で検索すれば、その部がこのサービスを使っていることが分かる。また **10人規模では、public投稿の著者表示名から実質的な名簿が再構成されうる**（architect の自己反論）。これに対しては、公開面に出る人名は**本人が明示的に昇格させた＝同意済み**のものだけであり網羅的名簿とは質が違う、と整理したうえで、**メンバー数・集計値を公開レスポンスの禁止キーに含めて**再構成を助長しない設計とした。

**詳細設計**: `docs/dev-org/SPEC-share2-team-pages.md` §9（秘匿レベル対応表・認可マトリクス・機械検証の項目）
